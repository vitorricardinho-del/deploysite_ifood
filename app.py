...
@app.route('/vitrine')
def vitrine():
    # 1. Buscamos do banco apenas os parceiros que foram aprovados (aprovado = True)
    resposta = supabase.table("categorias").select("*").eq("aprovado", True).execute()
    lojas_aprovadas = resposta.data if resposta.data else []
    
    # 2. Enviamos como 'parceiros' e como 'lojas' para blindar contra qualquer nome no HTML
...
@app.route('/acesso-restrito-parceiro-ms', methods=['GET', 'POST'])
def login():
...
def cadastrar_parceiro():
    ...
    if arquivo and arquivo.filename != '':
        ext = os.path.splitext(arquivo.filename)[1].lower()
        nome_da_foto = f"perfil_{int(time.time())}{ext}"
    ...
    # 2. Montando o dicionário sem a coluna inexistente 'categoria'
    dados_lanchonete = {
        "nome": nome_loja,
        ...
        "descricao": desc_loja,
        "foto_perfil": nome_da_foto,
        "status_online": False,
        "cor_tema": cor_tema if cor_tema else "#EA1D2C",
        "cidade": cidade_loja,      # Linha mantida (você já criou essa coluna!)
        "aprovado": False          # Linha mantida (você já criou essa coluna!)
    }
